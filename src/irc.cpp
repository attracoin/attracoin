...
void ThreadIRCSeed()
{
    ...
    for(;;)
    {
        ...
        if (fTestNet)
        {
            Send(hSocket, "JOIN #attracoinTEST\r");
            Send(hSocket, "WHO #attracoinTEST\r");
        } else
        {
            // randomly join #attracoin00-#attracoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #attracoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #attracoin%02d\r", channel_number).c_str());
        };

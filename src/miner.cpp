...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    while (true)
    {
        ...
        if (fTryToSync)
        {
			...
			LogPrintf("StakeMiner() vNodes.size = %d, nBestHeight = %d, GetNumBlocksOfPeers = %d\n", vNodes.size(), nBestHeight, GetNumBlocksOfPeers());
...
void ThreadPowMiner(CWallet *pwallet)
{
    ...
    while (true)
    {
        ...
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            fTryToSync = true;
            if (fDebugPoS)
                LogPrintf("ThreadPowMiner() IsInitialBlockDownload\n");
            MilliSleep(2000);
            boost::this_thread::interruption_point();
        };
        ...
        if (fTryToSync)
        {
            ...
            if (vNodes.size() < 3 )
            {
                fIsStaking = false;
                if (fDebugPoS)
                    LogPrintf("ThreadPowMiner() TryToSync\n");
                MilliSleep(60000);
                continue;
            };
        };
		...
		LogPrintf("ThreadPowMiner new proof-of-work block found  \n  hash: %s  \ntarget: %s\n", hashBlock.GetHex().c_str(), hashTarget.GetHex().c_str());

		CheckWork(pblock, *pwallet, reservekey);
 
        MilliSleep(nMinerSleep*2*60);
    };
}
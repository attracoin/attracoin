...
AttraCoinGUI::AttraCoinGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new AttraCoinBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("AttraCoin") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/attracoin"));
    setWindowIcon(QIcon(":icons/attracoin"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/attracoin/index.html");
#else
    QFile html("/opt/attracoin/index.html");
#endif
...
void AttraCoinGUI::pageLoaded(bool ok)
{
...
void AttraCoinGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);
}

void AttraCoinGUI::urlClicked(const QUrl & link)
{
...
void AttraCoinGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About AttraCoin"), this);
    aboutAction->setToolTip(tr("Show information about AttraCoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for AttraCoin"));
...
void AttraCoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void AttraCoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("AttraCoin") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/attracoin_testnet"));
            setWindowIcon(QIcon(":icons/attracoin_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/attracoin_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("AttraCoin client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/attracoin_testnet"));
...
void AttraCoinGUI::setWalletModel(WalletModel *walletModel)
{
...
void AttraCoinGUI::setMessageModel(MessageModel *messageModel)
{
...
void AttraCoinGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setToolTip(tr("AttraCoin client"));
    trayIcon->setIcon(QIcon(":/icons/attracoin"));
...
#ifndef Q_OS_MAC
void AttraCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void AttraCoinGUI::aboutClicked()
{
    AboutDialog dlg;
...
void AttraCoinGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("title", tr("%n active connection(s) to AttraCoin network", "", count));
}

void AttraCoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void AttraCoinGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void AttraCoinGUI::changeEvent(QEvent *e)
{
...
void AttraCoinGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void AttraCoinGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
...
void AttraCoinGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void AttraCoinGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void AttraCoinGUI::gotoSignMessageTab(QString addr)
{
...
void AttraCoinGUI::gotoVerifyMessageTab(QString addr)
{
...
void AttraCoinGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void AttraCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AttraCoinGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void AttraCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid AttraCoin address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void AttraCoinGUI::handleURI(QString strURI)
{
...
void AttraCoinGUI::setEncryptionStatus(int status)
{
...
void AttraCoinGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void AttraCoinGUI::backupWallet()
{
...
void AttraCoinGUI::changePassphrase()
{
...
void AttraCoinGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void AttraCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void AttraCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AttraCoinGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void AttraCoinGUI::updateStakingIcon()
{
...
void AttraCoinGUI::detectShutdown()
{

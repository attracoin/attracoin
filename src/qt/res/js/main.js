...
var unit = {
    ...
    setType: function(type) {
        ...
        switch(type) {
            ...
            case 2:
                this.name = "uATT",
                this.display = "&micro;ATT";
                break;

            case 3:
                this.name    = "sATT",
                this.display = "ATTtoshi";
                break;

            default:
                this.name = this.display = "ATT";
        }
...
var overviewPage = {
    init: function() {
        ...
        this.attracoinBal = $("#attracoinBal"),
        ...
        // Announcement feed
        $.ajax({
            url:"https://ajax.googleapis.com/ajax/services/feed/load?v=1.0&q=http://updates.attracoin.co/rss",
            dataType: 'jsonp'
        ...
        menu = [{
                 ...
                 {
                     name: ' About&nbsp;AttraCoin...',
                     img: 'qrc:///icons/attracoin',
                     fun: function () {
    ...
    updateBalance: function(balance, attracoinBal, stake, unconfirmed, immature) {
        if(balance == undefined)
            balance     = this.balance    .data("orig"),
            attracoinBal   = this.attracoinBal  .data("orig"),
            ...
            this.attracoinBal  .data("orig", attracoinBal),
        ...
        this.formatValue("attracoinBal",   attracoinBal);
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" title="The address to send the payment to  (e.g. PzjjetSdTwrppUwwNUo1GFHYTibzJi77jM)" placeholder="Enter an AttraCoin address (e.g. PzjjetSdTwrppUwwNUo1GFHYTibzJi77jM)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="AttraCoin"                    ' + (unit.type == 0 ? "selected" : "") + '>ATT</option> \
                    <option value="1" title="Milli-AttraCoin (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mATT</option> \
                    <option value="2" title="Micro-AttraCoin (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;ATT</option> \
                    <option value="3" title="ATTtoshi (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>ATTtoshi</option> \
                </select> \
            </div> \

...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return AttraCoinGUI::tr("Received with");
    case RecvFromOther:
        return AttraCoinGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return AttraCoinGUI::tr("Sent to");
    case SendToSelf:
        return AttraCoinGUI::tr("Payment to yourself");
    case Generated:
        return AttraCoinGUI::tr("Mined");
    case RecvAttraCoin:
        return AttraCoinGUI::tr("Received attracoin");
    case SendAttraCoin:
        return AttraCoinGUI::tr("Sent attracoin");
    case Other:
        return AttraCoinGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvAttraCoin:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendAttraCoin:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredOK, nCredAttraCoin;
    wtx.GetCredit(nCredOK, nCredAttraCoin, true);
    int64_t nCredit = nCredOK + nCredAttraCoin;
    ...
    {
        if (nNet > 0 && nCredAttraCoin > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvAttraCoin, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendAttraCoin, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvAttraCoin;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv attracoin
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendAttraCoin;
                    sub.address = CBitcoinAddress(ckidD).ToString();

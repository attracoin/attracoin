...
AttraCoinBridge::AttraCoinBridge(AttraCoinGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
AttraCoinBridge::~AttraCoinBridge()
{
...
void AttraCoinBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void AttraCoinBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void AttraCoinBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void AttraCoinBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void AttraCoinBridge::paste() {
    emitPaste(QApplication::clipboard()->text());
}

// Options
void AttraCoinBridge::populateOptions()
{
...
// Transactions
bool AttraCoinBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void AttraCoinBridge::clearRecipients()
{
    recipients.clear();
}

bool AttraCoinBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    ...
    {
        int inputType; // 0 OK, 1 AttraCoin
        switch(rcp.txnTypeInd)
        {
            case TXT_OK_TO_OK:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ATT, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_OK_TO_ANON:
                formatted.append(tr("<b>%1</b> to OKCASH %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ATT, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b>, ring size %2 to OKCASH %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ATT, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_OK:
                formatted.append(tr("<b>%1</b>, ring size %2 to OK %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ATT, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ATT, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsAttraCoinBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your attracoin balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ATT, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void AttraCoinBridge::openAddressBook(bool sending)
{
...
void AttraCoinBridge::openCoinControl()
{
...
void AttraCoinBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void AttraCoinBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap AttraCoinBridge::listAnonOutputs()
{
...
void AttraCoinBridge::populateTransactionTable()
{
...
void AttraCoinBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void AttraCoinBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString AttraCoinBridge::transactionDetails(QString txid)
{
...
// Addresses
void AttraCoinBridge::populateAddressTable()
{
...
void AttraCoinBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void AttraCoinBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString AttraCoinBridge::newAddress(bool own)
{
...
QString AttraCoinBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void AttraCoinBridge::updateAddressLabel(QString address, QString label)
{
...
bool AttraCoinBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool AttraCoinBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void AttraCoinBridge::appendMessages(QString messages, bool reset)
{
    emitMessages("[" + messages + "]", reset);
}

void AttraCoinBridge::appendMessage(int row) {
...
void AttraCoinBridge::populateMessageTable()
{
...
void AttraCoinBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool AttraCoinBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool AttraCoinBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString AttraCoinBridge::getPubKey(QString address, QString label)
{
    if(!label.isEmpty())
...
bool AttraCoinBridge::setPubKey(QString address, QString pubkey)
{
...
bool AttraCoinBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
    WalletModel::UnlockContext ctx(window->walletModel->requestUnlock());
...
void AttraCoinBridge::connectSignals()
{
...
QVariantMap AttraCoinBridge::userAction(QVariantMap action)
{
    ...
    if(key == "optionsChanged")
    {
        ...
        for(int option = 0;option < optionsModel->rowCount(); option++)
            if(value.contains(optionsModel->optionIDName(option))) {
				LogPrintf("MBC AttraCoinBridge::userAction value = %d/%d\n", optionsModel->index(option), value.value(optionsModel->optionIDName(option)) );  //MBC
...
// Blocks
QVariantMap AttraCoinBridge::listLatestBlocks()
{
...
QVariantMap AttraCoinBridge::findBlock(QString searchID)
{
...
QVariantMap AttraCoinBridge::blockDetails(QString blkHash)
{
...
QVariantMap AttraCoinBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap AttraCoinBridge::txnDetails(QString blkHash, QString txnHash)
{

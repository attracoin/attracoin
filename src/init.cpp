...
{
    uiInterface.ThreadSafeMessageBox(str, _("AttraCoin"), CClientUIInterface::BTN_OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("AttraCoin"), CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    std::string strUsage = _("Options:") + "\n";
        ...
        strUsage += "  -conf=<file>           " + _("Specify configuration file (default: attracoin.conf)") + "\n";
        strUsage += "  -pid=<file>            " + _("Specify pid file (default: attracoindv2.pid)") + "\n";
        ...
        strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 6370 or testnet: 7380)") + "\n";
        ...
        strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 6369 or testnet: 7379)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. AttraCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  AttraCoin is probably already running."), strDataDir.c_str()));
    
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();

    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("AttraCoin version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
    {
        fprintf(stdout, "AttraCoin server starting\n");
        fflush(stdout);
    };
    ...
    ********************************************************* */
    ...
    if (fs::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("AttraCoin"), CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING | CClientUIInterface::MODAL);
        };
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        {
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of AttraCoin") << "\n";
        } else
        if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart AttraCoin to complete") << "\n";
	...
	//MBC 2018.05.11  POW create block
	if (!GetBoolArg("-gen", true))
        LogPrintf("POW create block disabled\n");
    else
        threadGroup.create_thread(boost::bind(&TraceThread<void (*)(CWallet*), CWallet*>, "powminer", &ThreadPowMiner, pwalletMain));

...
const string strMessageMagic = "AttraCoinV2 Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        LogPrintf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "AttraCoin", CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    };
    return true;
    ...
    while (it != pfrom->vRecvGetData.end())
    {
            ...
            || inv.type == MSG_FILTERED_BLOCK)
        {
            ...
            if (mi != mapBlockIndex.end())
            {
				...
				//LogPrintf("MBC ProcessGetData nHeight = %d \n", nHeight);
            ...
            if (send)
            {
                ...
                if (inv.type == MSG_BLOCK)
                {
                    ...
                    {
						if (pfrom->nVersion == MIN_PEER_SILVER_VERSION)    //MBC 20180623
						{
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
		...
		//LogPrintf("version: 31 \n");


        if (!vRecv.empty())
            vRecv >> pfrom->strSubVer;
        
		//LogPrintf("version: 32 \n");

		if (!vRecv.empty())
            vRecv >> pfrom->nChainHeight;

		//LogPrintf("version: 33 \n");
        ...
        {	
			if (pfrom->nVersion == MIN_PEER_SILVER_VERSION) {    //MBC 20180623
				if (pindex) {
					if(pindex->nHeight > 1) {
						pindex = pindex->pnext;
						LogPrintf("  getblocks pfrom->nVersion == MIN_PEER_SILVER_VERSION\n");
					}
				}
			}else{
				if (pindex) {				
    ...
    if (strCommand == "getheaders")
    {
      ...
      //      return false;
        ...
        {	
			...
			//LogPrintf("getheaders silver (ver=%d) %d to %s\n", pfrom->nVersion, (pindex ? pindex->nHeight : -1), hashStop.ToString().substr(0,20).c_str());
			for (; pindex; pindex = pindex->pnext)
			{
			...
			int nLimit = MAX_GETHEADERS_SZ;
			//LogPrintf("getheaders (ver=%d) %d to %s\n", pfrom->nVersion, (pindex ? pindex->nHeight : -1), hashStop.ToString().substr(0,20).c_str());
			for (; pindex; pindex = pindex->pnext)
			{

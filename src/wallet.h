...
public:
    ...
    int64_t GetAttraCoinBalance() const;
    ...
    int64_t Getattracoindv2ebit(const CTxIn& txin) const;
    int64_t GetAttraCoinCredit(const CTxOut& txout) const;
    ...
    int64_t GetDebit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txin.IsAnonInput())
            {
                nDebit += Getattracoindv2ebit(txin);
            } else
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetAttraCoinCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nOK, int64_t& nAttraCoin) const
    {
        nOK = 0;
        nAttraCoin = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nAttraCoin += GetAttraCoinCredit(txout);
            } else
                nOK += GetCredit(txout);
            if (!MoneyRange(nOK)
                || !MoneyRange(nAttraCoin))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
        return true;
    };
...
public:
    ...
    mutable bool fAvailableAttraCoinCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableAttraCoinCreditCached;
    
    mutable int64_t nCredOKCached;
    mutable int64_t nCredAttraCoinCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableAttraCoinCreditCached = false;
        
        nCredOKCached = 0;
        nCredAttraCoinCached = 0;
        ...
        nAvailableAttraCoinCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredOK, int64_t& nCredAttraCoin, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredOK = nCredAttraCoin = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredOKCached = 0;
            nCredAttraCoinCached = 0;
            pwallet->GetCredit(*this, nCredOKCached, nCredAttraCoinCached);
            fCreditSplitCached = true;
        };
        
        nCredOK = nCredOKCached;
        nCredAttraCoin = nCredAttraCoinCached;
        return true;
    }
    ...
    int64_t GetAvailableAttraCoinCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableAttraCoinCreditCached)
            return nAvailableAttraCoinCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableAttraCoinCredit() : value out of range");
            };
        };
        
        nAvailableAttraCoinCreditCached = nCredit;
        fAvailableAttraCoinCreditCached = true;
        return nCredit;
    };
